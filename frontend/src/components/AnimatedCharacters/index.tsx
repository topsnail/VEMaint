...
export function AnimatedCharacters({
  ...
  // 隐私保护：当密码框聚焦且未“显示密码”时进入隐藏模式
  // - 轻度隐私：已聚焦但尚未输入
  // - 强度隐私：已聚焦且已输入（遮挡更明显）
  const isHidingPassword = passwordFocused && !showPassword;
  const isHidingStrong = isHidingPassword && passwordLength > 0;
  const isHidingSoft = isHidingPassword && passwordLength <= 0;
  // 可见模式：当密码框聚焦且“显示密码”时进入展示模式
  const isShowingPassword = passwordFocused && showPassword;
    ...
    const tick = () => {
      ...
      if (purpleRef.current && !showing) {
        ...
        if (typing) {
          ...
          qt.purpleX(40);
          qt.purpleHeight(440);
        } else if (hiding && stateRef.current.isHidingStrong) {
          // 强度隐私：已输入密码，遮挡更明显
          qt.purpleSkew(pp.bodySkew - 18);
          qt.purpleX(46);
          qt.purpleHeight(450);
        } else if (hiding) {
          // 轻度隐私：聚焦但未输入
      ...
      if (blackRef.current && !showing) {
        ...
        if (looking) {
          ...
          qt.blackSkew(bp.bodySkew * 1.35);
          qt.blackX(0);
        } else if (hiding) {
          qt.blackSkew(bp.bodySkew * 1.15);
          qt.blackX(0);
        } else {
  ...
  const applyHidingPassword = contextSafe(() => {
    const qt = quickToRef.current;
    if (qt) {
      // 强度隐私（已有输入）会让“表情更不情愿”，以增强遮挡感
      const strong = passwordLength > 0;
      qt.purpleFaceLeft(strong ? 55 : 50);
      qt.purpleFaceTop(strong ? 65 : 60);
    }
  });
  ...
  }, [isHidingPassword, isShowingPassword, isHidingStrong]);
